#!/usr/bin/env python3
"""Deploy one reviewed Phase 4 artifact; no configuration, DNS or production changes.
Run with sudo on the verified VPS after CI/artifact comparison. Args: SHA envelope-SHA256 CI-run.
The artifact envelope must be build-standard-artifact.json beside this script.
"""
import base64
...
OLD = ROOT / 'releases/1a0909e9e3417c5eaf1f48cafc52da1be9a2d4ee'
...
def main():
    ...
    raw = (source / 'build-standard-artifact.json').read_bytes()
    ...
    assert meta == {'version': '0.1.0-dev.4', 'commit': sha, 'dirty': False, 'tag': None, 'source': 'https://github.com/jimlunsford/only-empowerment/commit/' + sha}
    ...
    helper = private / ('deploy-build-standard-' + sha + '.py')
    shutil.copy2(Path(__file__), helper)
    helper.chmod(0o600)
    def switch(target):
        temp = ROOT / 'current.build-standard'
    ...
    try:
        ...
        raise
    (source / 'build-standard-deployment-result.json').write_text(json.dumps(d.EVIDENCE, indent=2) + '\n')
    print('BUILD A STANDARD CANDIDATE STAGED: ' + sha, flush=True)
