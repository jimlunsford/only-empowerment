#!/usr/bin/env python3
"""Deploy one reviewed Phase 7 artifact; no configuration, DNS or production changes.
Run with sudo on the verified VPS after CI/artifact comparison. Args: SHA envelope-SHA256 CI-run.
The artifact envelope must be action-artifact.json beside this script.
"""
import base64
...
def main():
    ...
    raw = (source / 'action-artifact.json').read_bytes()
    ...
    assert meta == {'version': '0.1.0-dev.7', 'commit': sha, 'dirty': False, 'tag': None, 'source': 'https://github.com/jimlunsford/only-empowerment/commit/' + sha}
    ...
    helper = private / ('deploy-do-it-now-' + sha + '.py')
    shutil.copy2(Path(__file__), helper)
    helper.chmod(0o600)
    def switch(target):
        temp = ROOT / 'current.action'
    ...
    try:
        ...
        raise
    (source / 'action-deployment-result.json').write_text(json.dumps(d.EVIDENCE, indent=2) + '\n')
    print('DO IT NOW CANDIDATE STAGED: ' + sha, flush=True)
