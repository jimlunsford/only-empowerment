#!/usr/bin/env python3
"""Deploy one reviewed Phase 2 artifact; no configuration, DNS or production changes.
Run with sudo on the verified VPS after CI/artifact comparison. Args: SHA envelope-SHA256 CI-run.
The artifact envelope must be next-move-artifact.json beside this script.
"""
import base64
...
OLD = ROOT / 'releases/bd984a8986a658c6bd0d0d663c4e30f5c2f09d80'
...
def main():
    ...
    raw = (source / 'next-move-artifact.json').read_bytes()
    ...
    assert meta == {'version': '0.1.0-dev.2', 'commit': sha, 'dirty': False, 'tag': None, 'source': 'https://github.com/jimlunsford/only-empowerment/commit/' + sha}
    ...
    helper = private / ('deploy-next-move-' + sha + '.py')
    shutil.copy2(Path(__file__), helper)
    helper.chmod(0o600)
    def switch(target):
        temp = ROOT / 'current.next-move'
    ...
    try:
        ...
        raise
    (source / 'next-move-deployment-result.json').write_text(json.dumps(d.EVIDENCE, indent=2) + '\n')
    print('NEXT MOVE CANDIDATE STAGED: ' + sha, flush=True)
