#!/usr/bin/env python3
"""Deploy one reviewed Phase 6 artifact; no configuration, DNS or production changes.
Run with sudo on the verified VPS after CI/artifact comparison. Args: SHA envelope-SHA256 CI-run.
The artifact envelope must be rebuild-artifact.json beside this script.
"""
import base64
...
OLD = ROOT / 'releases/396a7ea49a41fcc61fb3a0ab0402acad74a9dff1'
...
def main():
    ...
    raw = (source / 'rebuild-artifact.json').read_bytes()
    ...
    assert meta == {'version': '0.1.0-dev.6', 'commit': sha, 'dirty': False, 'tag': None, 'source': 'https://github.com/jimlunsford/only-empowerment/commit/' + sha}
    ...
    helper = private / ('deploy-rebuild-' + sha + '.py')
    shutil.copy2(Path(__file__), helper)
    helper.chmod(0o600)
    def switch(target):
        temp = ROOT / 'current.rebuild'
    ...
    try:
        ...
        raise
    (source / 'rebuild-deployment-result.json').write_text(json.dumps(d.EVIDENCE, indent=2) + '\n')
    print('REBUILD MAP CANDIDATE STAGED: ' + sha, flush=True)
