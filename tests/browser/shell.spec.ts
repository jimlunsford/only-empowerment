...
const routes = ['/', '/tools', '/approach', '/privacy', '/preview', '/tools/decision-room', '/tools/next-move', '/tools/reset', '/tools/build-a-standard', '/tools/rebuild-map', '/tools/do-it-now'];
test('all shell routes are accessible and fit the viewport', async ({page}) => {
...
test('preview validates, escapes text, edits, and clears in-memory work', async ({page}) => {
  ...
  await page.getByLabel('What is one action you could finish?').fill('<img src=x onerror=alert(1)> Send one question.');
  await page.getByRole('button',{name:'Review the sample card'}).click();
  ...
  await expect(page.getByLabel('What is one action you could finish?')).toHaveValue(/Send one question/);
...
test('private marker stays out of requests and storage; reload and exit discard it', async ({page, context}) => {
  ...
  expect(requests.every(r => r.method === 'GET' && new URL(r.url).origin === 'http://127.0.0.1:4173' && !JSON.stringify(r).includes(marker))).toBe(true);
...
test('source footer and metadata identify the same build', async ({page,request}) => {
  ...
  if (!metadata.dirty) await expect(page.locator('.footer-meta a').last()).toHaveAttribute('href',metadata.source);
});
